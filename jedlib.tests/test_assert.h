...
#define EXPECT_EQ_STR(exp, val) TestEqStr(exp, val, __FILE__, __LINE__)

#ifdef _WIN32
#define EXPECT_TRUE(val) TestEq(true, val, __FILE__, __LINE__, __FUNCTION__)
#define EXPECT_EQ(exp, val) TestEq(exp, val, __FILE__, __LINE__, __FUNCTION__)
#define EXPECT_NEAR(exp, val, tol) TestEqClose(exp, val, tol, __FILE__, __LINE__, __FUNCTION__)
#else
#define EXPECT_TRUE(val) TestEq(true, val, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#define EXPECT_EQ(exp, val) TestEq(exp, val, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#define EXPECT_NEAR(exp, val, tol) TestEqClose(exp, val, tol, __FILE__, __LINE__, __PRETTY_FUNCTION__)
#endif
