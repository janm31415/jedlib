...
file_buffer save_to_file(bool& success, file_buffer fb, const std::string& filename)
  {
#ifdef _WIN32
  std::wstring wfilename = convert_string_to_wstring(filename); // filenames are in utf8 encoding
#else
  std::string wfilename(filename);
#endif
  success = false;
...
int64_t get_col_from_line_length(line ln, int64_t length, const env_settings& s)
  ...
  int64_t col = 0;
  int64_t out = 0;
  for (uint32_t i = 0; le < length && i < ln.size(); ++i)
    {
    uint32_t w = character_width(ln[i], col, s);
    le += w;
    col += w;
...
bool has_multiline_selection(file_buffer fb)
  ...
  if (fb.start_selection && (*fb.start_selection != fb.pos))
    {
    return fb.pos.row != fb.start_selection->row;
    }
  return false;
  }
...
file_buffer insert(file_buffer fb, const std::string& txt, const env_settings& s, bool save_undo)
  {
  std::wstring wtxt = convert_string_to_wstring(txt);
    ...
    if (p1.row == p2.row)
      ...
      if (!fb.rectangular_selection)
        ...
        if (tgt > fb.content[p2.row].size() - 1)
          remove_line = true;
        fb.content = fb.content.set(p2.row, fb.content[p2.row].erase(0, static_cast<uint32_t>(tgt)));
        fb.content = fb.content.erase(p1.row + 1, remove_line ? p2.row + 1 : p2.row);
        fb.lex = fb.lex.erase(p1.row + 1, remove_line ? p2.row + 1 : p2.row);
        fb.content = fb.content.set(p1.row, fb.content[p1.row].erase(p1.col, static_cast<int64_t>(fb.content[p1.row].size()) - 1));
...
std::string to_string(text txt, position from, position to) {
  return convert_wstring_to_string(to_wstring(txt, from, to));
  }
...
text to_text(const std::string& txt)
  {
  return to_text(convert_string_to_wstring(txt));
  }
